...
namespace Drupal\vmware\Plugin\Derivative;
 ...
 *
 * @see \Drupal\vmware\Plugin\Derivative\VmwareMenuLinks
 */
class VmwareMenuLinks extends DeriverBase implements ContainerDeriverInterface {
  ...
  /**
   * Constructs new VmwareMenuLinks.
   *
  ...
  public function getDerivativeDefinitions($base_plugin_definition) {
    // Get all VMware entities.
    $entities = $this->cloudConfigPluginManager->loadConfigEntities('vmware');
    $links = [];
    $weight = 300;
    $entity_types = [
      'vmware_vm',
    ...
    if (!empty($entities)) {
      // Add dropdown menu for all VMware VMs.
      $entity_type = 'vmware_vm';
      $entity_definition = $this->entityTypeManager->getDefinition($entity_type);
      $title = $entity_definition->getPluralLabel();
      $id = "vmware.local_tasks." . $entity_type;
      $links[$id] = $base_plugin_definition;
      $links[$id]['title'] = $this->t('VMware @title', ['@title' => $title]);
      ...
      // Add dropdown menu for VMware.
      $title = "VMware";
      $id = "vmware.service_providers.menu";
      $links[$id] = $base_plugin_definition;
      $links[$id]['title'] = $this->t('@title', ['@title' => $title]);
      $links[$id]['route_name'] = "view.cloud_config.list";
      $links[$id]['menu_name'] = 'vmware.service_providers.menu';
    ...
    foreach ($entities ?: [] as $entity) {
      ...
      // Add menu items for vmware cluster.
      $menu_data = [];
      $menu_data[$base_id] = [
  ...
  private function addMenuItems(
    ...
    foreach ($menu_data ?: [] as $id => $link_data) {
      $links[$id] = $base_plugin_definition;
      $links[$id]['base_route'] = 'vmware.service_providers.menu';
      $links[$id]['parent'] = 'vmware.menu.cloud_context:vmware.service_providers.menu';
      $links[$id]['expanded'] = TRUE;
  ...
  private function addChildItems(
    ...
    foreach ($entity_types ?: [] as $entity_type) {
      ...
      $title = preg_replace('/VMware (.*)/', '${1}', $label);
      ...
      $links[$id]['parent'] = "vmware.menu.cloud_context:$parent_link_id";
