...
class CloudWatchService extends CloudServiceBase implements CloudWatchServiceInterface {
  ...
  public function getMetricData(array $params = []) {
    $results = $this->execute('GetMetricData', $params);
    return $results;
  }
  ...
  private function execute($operation, array $params = []) {
    ...
    $cloud_watch_client = $this->getCloudWatchClient();
   ...
   */
  private function addMockHandler(CloudWatchClient $cloud_watch_client) {
    ...
    if ($this->dryRun && $mock_data) {
      ...
      // Set mock handler.
      $cloud_watch_client->getHandlerList()->setHandler(new MockHandler([$func, $func]));
    }
  }
