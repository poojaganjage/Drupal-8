...
class InstanceTypePriceTableRenderer {
  ...
  public function render($cloud_context, $instance_type = NULL) {
    ...
    $table_header = array_map(static function ($key, $value) {
      return [
        'data' => $value,
        'field' => $key,
      ];
   ...
   */
  private function getTableRows(array $rows_data, $instance_type) {
    $rows = [];
    foreach ($rows_data ?: [] as $row_data) {
      ...
      $row['#attributes']['class'] = [
        str_replace('.', '_', $row_data['instance_type']),
      ];
