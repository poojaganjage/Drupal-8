...
class SecurityGroupRevokeForm extends AwsDeleteForm {
  ...
  public function getQuestion() {
    ...
    return $this->t('Are you sure you want to revoke the following @type permission?',
      [
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($permission !== FALSE) {
      ...
      if ($type === 'outbound_permission') {
        ...
        $this->ec2Service->revokeSecurityGroupEgress($perm_array);
      }
      else {
        $perm_array['IpPermissions'][] = $this->formatIpPermissionForRevoke($permission);
        $this->ec2Service->revokeSecurityGroupIngress($perm_array);
      }
   ...
   *   True or false.
   */
  private function validateRevoke($type, CloudContentEntityBase $group) {
    $verified = FALSE;
   ...
   */
  private function getPermission($type, $position, CloudContentEntityBase $entity) {
    $permission = FALSE;
    switch ($type) {
