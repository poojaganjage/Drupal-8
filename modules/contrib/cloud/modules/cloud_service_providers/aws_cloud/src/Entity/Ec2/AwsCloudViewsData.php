 ...
 */
class AwsCloudViewsData extends EntityViewsData implements EntityViewsDataInterface {
    ...
    foreach ($fields ?: [] as $key => $field) {
      ...
      if (in_array($key, $selectable)) {
        $data[$table_name][$key . '_fs'] = [
          'title' => $this->t('@label (selector)', [
            '@label' => $field->getLabel(),
          ]),
          'help' => $this->t('Provides a dropdown option for text filtering.  If there are no results, the filter will not be shown.'),
          'filter' => [
            'field' => $key,
            'table' => $table_name,
            'id' => 'texttoselect',
