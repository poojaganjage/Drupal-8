...
class RateWidgetField extends FieldPluginBase {
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    ...
    foreach ($widgets as $id => $widget) {
      ...
      if (count($widget_entities > 0)) {
        foreach ($widget_entities as $index => $entity) {
  ...
  public function render(ResultRow $row) {
    ...
    if (!isset($entity_id) || !isset($entity_type_id) || !isset($bundle)) {
      ...
      $form = $rate_widget_base_service->getForm($entity_type_id, $bundle, $entity_id, $vote_type, $widget_name, $widget);
